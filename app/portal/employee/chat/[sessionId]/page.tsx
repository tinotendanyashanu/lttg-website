...
import EmployeeChatReply from "./EmployeeChatReply";
...
const ROLE_CONFIG: Record<MessageRole, { bg: string; text: string; bubble: string }> = {
  bot: { bg: "bg-[#7c3aed]/10", text: "text-[#7c3aed]", bubble: "bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100" },
  visitor: { bg: "bg-gray-100 dark:bg-gray-700", text: "text-gray-500", bubble: "bg-[#7c3aed] text-white" },
  employee: { bg: "bg-emerald-100 dark:bg-emerald-900", text: "text-emerald-700 dark:text-emerald-300", bubble: "bg-emerald-50 dark:bg-emerald-950 text-emerald-900 dark:text-emerald-100 border border-emerald-200 dark:border-emerald-800" },
};

async function getChatSession(id: string): Promise<ChatSessionDetail | null> {
  await dbConnect();
...
export default async function EmployeeChatSessionPage({
  params,
}: {
  ...
  if (!session?.user?.email) redirect("/login?loginSource=portal");

  const account = await getAccountByEmail(session.user.email);
  if (!account || account.isActive === false) redirect("/login?loginSource=portal");

  const canReply = account.roles.includes("admin") || account.roles.includes("employee");
  if (!canReply) redirect("/portal/employee");
  ...
  const statusColors: Record<ChatStatus, string> = {
    bot: "bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300",
    pending_human: "bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300",
    human_active: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300",
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10 max-w-3xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#27272a] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5">
        ...
        <div className="mt-3 grid grid-cols-3 gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-gray-900 dark:text-white">{new Date(chatSession.createdAt).toLocaleDateString()}</p>
          </div>
          <div>
        ...
        <div className="mt-4 rounded-xl bg-gray-50 dark:bg-gray-800/60 p-3 text-xs text-gray-500 dark:text-gray-400 space-y-1">
          ...
          {chatSession.assignedEmployeeName && <p><span className="font-semibold text-gray-700 dark:text-gray-200">Assigned:</span> {chatSession.assignedEmployeeName}</p>}
      ...
      <div className="bg-white dark:bg-[#27272a] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-50 dark:divide-gray-800/50 max-h-[460px] overflow-y-auto">
          {messages.map((m, i) => {
      ...
      {chatSession.status !== "resolved" ? (
        <EmployeeChatReply sessionId={chatSession._id} />
      ) : (
