...
import { getQuestsForAdmin } from '@/lib/actions/quests';
import QuestsManagerClient from './QuestsManagerClient';

export const metadata = {
  title: 'Sales Quests | LeoTech Admin',
};
...
export default async function AdminQuestsPage() {
  ...
  const quests = await getQuestsForAdmin();
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-brand-primary text-white rounded-2xl flex items-center justify-center shadow-lg shadow-brand-primary/20">
              <span className="material-icons-outlined text-2xl">military_tech</span>
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Sales quests</h1>
              <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">
                Set goals for your team. Progress appears on employee and intern dashboards.
              </p>
            </div>
          </div>
        </div>
      ...
      <QuestsManagerClient initialQuests={quests} />
    </div>
  );
}
