...
export const metadata = {
  title: 'Manage Clients | Admin Panel',
};
...
export default async function AdminClientsPage() {
  ...
  try {
    ...
    initialClients = response.clients;
  } catch (error) {
    redirect('/portal');
  }
      ...
      <div className="bg-white dark:bg-[#27272a] rounded-3xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Client Management Module</h1>
              <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">Search, edit, merge and archive system clients.</p>
            </div>
          </div>
        </div>
      </div>

      <ClientManagerClient initialClients={initialClients} />
    </div>
  );
}
