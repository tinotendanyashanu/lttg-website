...
import { updateContractTemplate } from '@/lib/actions/contract-templates';
...
export default function EditTemplateModal({ template, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await updateContractTemplate(template._id, { name, description, category, content });
        setSuccess(true);
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-brand-primary rounded-xl flex items-center justify-center">
              <span className="material-icons-outlined text-white text-[18px]">edit_note</span>
            </div>
            <div>
              <h2 className="text-base font-bold text-gray-900 dark:text-white">Edit Template</h2>
              <p className="text-xs text-gray-400">{template.name}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 p-1 rounded-lg transition-colors">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              >
                <span className="material-icons-outlined text-[14px]">{preview ? 'code' : 'visibility'}</span>
            ...
            <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1">
              Write HTML. Use {'{{variable}}'} placeholders.
            </p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            <button type="button" onClick={onClose} disabled={isPending} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
              Cancel
            </button>
            <button
              type="submit"
            ...
            >
              {isPending ? (
                ...
                <><span className="material-icons-outlined text-[16px]">save</span>Save Changes</>
              )}
            </button>
          </div>
        </form>
